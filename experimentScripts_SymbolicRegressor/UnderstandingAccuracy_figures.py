...
def from_data_to_figures(df):
    ...
    ax.set_ylabel("Points per evaluation")
    ...
    list_total_times=list(df.groupby('accuracy')['n_eval'].sum())
    ...
    # Guardar imagen.
    plt.savefig('results/figures/SymbolicRegressor/UnderstandingAccuracy.png')
    plt.show()
    plt.close()
...
def from_data_to_figures_paper(df):
    ...
    ax.set_xticklabels(range(0,60,10),rotation=0)
    ...
    # Guardar imagen.
    plt.savefig('results/figures/SymbolicRegressor/UnderstandingAccuracy_paper.png')
    plt.show()
    plt.close()
