...
def learn(init_acc,default_train_n_pts,train_pts_seed,train_seed,df_test_pts,max_n_eval):
    ...
    est_surf.fit(init_acc,default_train_n_pts,train_pts_seed,xy_train, z_train,max_n_eval,train_seed,df_test_pts)    

    return est_surf._program 

# FUNCIÓN 5 (Calculo del incremento de accuracy)
# Parámetros:
...
def acc_split(corr,acc_rest):
	if corr<=0:
		split=acc_rest
	else:
		split=-acc_rest*(corr-1)
	return split

# FUNCIÓN 6 (Cálculo del coeficiente de correlación de Spearman)
...
# FUNCIÓN 7 (Transformar lista de scores en ranking)
...
# FUNCIÓN 8 (Evaluar un conjunto de puntos en cada superficie que forma una generación)
# Parámetros:
...
# -Script: genetic.py
# -Clase: BaseSymbolic
def new_fit(self,init_acc,default_train_n_pts,train_pts_seed, X, y, max_n_eval, train_seed,df_test_pts,sample_weight=None):# MODIFICACIÓN: añadir nuevos argumentos.
    ...
    while n_evaluations < max_n_eval:# MODIFICACIÓN: modificar el límite de entrenamiento.
        ...
        df_train.append([train_seed,acc,gen,score,elapsed_time,generation_time,n_evaluations])

        # MODIFICACIÓN: modificación automática del accuracy.
        if acc<1:
            # Guardar los scores asociados a cada superficie que forma la generación.
            list_surfaces=list(population)# Lista con las superficies que forman la generación. 
            current_scores=generation_score_list(list_surfaces,np.insert(X, X.shape[1], y, 1))# Con el accuracy actual. 
...
# Accuracy de partida (el correspondiente a un conjunto con un único punto).
init_acc=1/default_train_n_pts
...
for train_seed in tqdm(list_train_seeds):
    ...
    # Entrenamiento.
    expr_surf_pred=learn(init_acc,default_train_n_pts,train_pts_seed,train_seed,df_test_pts,max_n_eval)

df_train=pd.DataFrame(df_train,columns=['train_seed','acc','n_gen','score','elapsed_time','time_gen','n_eval'])
df_train.to_csv('results/data/SymbolicRegressor/df_train_AscendantAccuracy.csv')
...
np.save('results/data/SymbolicRegressor/init_acc_automatic_ascendant',init_acc)
