...
# FUNCIÓN 4
def learn(init_acc,split_acc,n_eval_to_change_accuracy,default_train_n_pts,train_pts_seed,train_seed,df_test_pts,max_n_eval):
    ...
    est_surf.fit(init_acc,split_acc,n_eval_to_change_accuracy,default_train_n_pts,train_pts_seed,xy_train, z_train,max_n_eval,train_seed,df_test_pts)    

    return est_surf._program 

# FUNCIÓN 5
def change_acc_true_or_false(self,n_evaluations,n_eval_to_change_accuracy,default_train_n_pts,acc):
    ...
    list_dif=n_evaluations-np.array(n_eval_to_change_accuracy)
    neg_dif=np.abs(list_dif[list_dif<=0])
    split=self.population_size*int(default_train_n_pts*acc)
...
# -Script: genetic.py
# -Clase: BaseSymbolic
def new_fit(self,init_acc,split_acc,n_eval_to_change_accuracy,default_train_n_pts,train_pts_seed, X, y, max_n_eval, train_seed,df_test_pts,sample_weight=None):# MODIFICACIÓN: añadir nuevos argumentos.
    ...
    while n_evaluations < max_n_eval:# MODIFICACIÓN: modificar el límite de entrenamiento.
        ...
        # MODIFICACIÓN:
        # Comprobar si hay que modificar el accuracy.
        change=change_acc_true_or_false(self,n_evaluations,n_eval_to_change_accuracy,default_train_n_pts,acc)
        
        # Modificar accuracy en caso de que toque.
        if change==True:
            # Calcular nuevo valor de accuracy (incrementar accuracy).
            acc=acc+split_acc
            if acc>1:
                acc=1
...
# Parámetros relacionados con el accuracy del tamaño del conjunto inicial de puntos.
init_acc=0.5# Valor inicial del accuracy.
split_acc=0.5# Incremento constante en el accuracy para cada actualización.
freq_change=250000
n_eval_to_change_accuracy=range(freq_change,max_n_eval+freq_change,freq_change)# Número de evaluaciones en los que se cambiará el valor de accuracy.
