'''
This script is an adaptation of script experimentScripts_MuJoCo/UnderstandingAccuracy_figures.py. 
...
def from_data_to_figures_paper(df):
    ...
    ax=plt.subplot(221)
    ax.set_title(r'\textbf{Swimmer}',fontsize=16)
    y=[i*(-1) for i in all_means]
    y.reverse()
    plt.barh(convert_textbf(list_acc_str), y, align='center',color=color[0])
    plt.ylabel("$c$",fontsize=16)
    plt.xlabel("$t_c$",fontsize=16)
    ax.set_xticks(range(0,-1500,-500))
    ax.set_xticklabels(convert_textbf(range(0,1500,500)),rotation=0)
    ...
    # Save graph.
    plt.savefig('figures_paper/figures/UnderstandingAccuracy/UnderstandingAccuracy_MuJoCo.png')
    plt.savefig('figures_paper/figures/UnderstandingAccuracy/UnderstandingAccuracy_MuJoCo.pdf')
    plt.show()
    plt.close()
...
# MAIN PROGRAM
#==================================================================================================
df=pd.read_csv('results/data/MuJoCo/UnderstandingAccuracy/df_UnderstandingAccuracy.csv',index_col=0)
