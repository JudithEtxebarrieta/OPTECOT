...
def from_data_to_figures_paper(df):
    ...
    ax=plt.subplot(221)
    ax.set_title(r'\textbf{WindFLO}',fontsize=16)
    y=[i*(-1) for i in all_means]
    y.reverse()
    plt.barh(convert_textbf(list_acc_str), y, align='center',color=color[0])
    plt.ylabel("$c$",fontsize=16)
    plt.xlabel("$t_c$",fontsize=16)
    ax.set_xticks(np.arange(0,-0.1,-0.03))
    ax.set_xticklabels(convert_textbf(np.arange(0,0.1,0.03)),rotation=0)
    ...
    # Save graph.
    plt.savefig('figures_paper/figures/UnderstandingAccuracy/UnderstandingAccuracy_WindFLO.png')
    plt.savefig('figures_paper/figures/UnderstandingAccuracy/UnderstandingAccuracy_WindFLO.pdf')
    plt.show()
    plt.close()
