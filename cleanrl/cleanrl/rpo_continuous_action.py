...
def parse_args(env_id, num_steps, total_timesteps):
    # fmt: off
...
def make_env(env_id, idx, capture_video, run_name, gamma, timestep):
    def thunk():
        if capture_video:
            env = gym.make(env_id, render_mode="rgb_array")
        else:
            env = gym.make(env_id)
        env.env.env.unwrapped.frame_skip = 1
        env.env.env.unwrapped.model.opt.timestep = timestep
