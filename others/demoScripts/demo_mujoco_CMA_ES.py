...
def rollout(self):
    ...
    i = -1 # Inicializar contador para sumar el numero de steps dados en este episodio.
    sum_of_rewards = 0 # Para sumar el numero de rewards en el actual episodio.
    episode_start_ref_t = time.time() # Inicializar contador para medir tiempo de este episodio.

    # Inicializar episodio.
    self.start_episode() 
    self._max_episode_length = max_episode_length # Fijar tamaño maximo del episodio.
    ...
    list_gen_times_per_episode.append(episode_runtime) # Tiempo de ejecucion gastado hasta el momento.
    n_episode += 1 # Episodios evaluados hasta el moemento.
    
    # Cuando se han evaluado los  suficientes episodios como para poder completar una generacion nueva,
    # se guarda la informacion obtenida de la nueva generacion en una nueva linea del fichero "res_filepath".
...
# PROGRAMA PRINCIPAL
#==================================================================================================
# Modificacion de funcion existente para introducir criterio de parada temprano.
